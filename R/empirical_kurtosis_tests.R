...
empirical_kurtosis_one_sample <- function(x, kurtosis, alternative = "two.sided", conf.level = .95) {
  ...
  calc_test_stat <- function(x, kurtosis, alternative) {
    ...
    obs_p <- calc_obs_p(x)
    null_p <- calc_null_p_kurtosis(x, kurtosis)
...
#' @details
#' \itemize{
#' \item Null: All excess kurtosis are equal. (kurtosis 1 = kurtosis 2 ... kurtosis k).
#' \item Alternative: At least one excess kurtosis is not equal.
#' }
#'
#' Because groups are independent, the -2 log likelihood ratios of the groups
#' add. The common excess kurtosis under the null is profiled out by
#' minimizing the summed statistic.
#'
...
#' roughly .08 to .11 at the nominal .05. Large groups are recommended.
#' @examples
#' library(LRTesteR)
#'
#' # Null is true
...
#' empirical_kurtosis_one_way(x, fctr, .95)
#' @export
empirical_kurtosis_one_way <- function(x, fctr, conf.level = 0.95) {
  if (length(x) < 1) {
  ...
  if (any(as.vector(by(x, fctr, length)) < 5)) {
    stop("Each group in x should have at least five observations.")
  }
  ...
  calc_test_stat <- function(x, fctr) {
    ests <- as.vector(by(x, fctr, function(z) mean((z - mean(z))^4) / mean((z - mean(z))^2)^2 - 3))

    # The common excess kurtosis is profiled out. The minimizer lies near
    # the hull of the group estimates, so the search interval is the range
    # of group estimates with a margin. Excess kurtosis is bounded below
    # by negative two.
    total_W <- function(theta) {
      Ws <- vapply(
        levels(fctr),
        function(l) calc_group_W(x[fctr == l], theta, calc_null_p_kurtosis),
        numeric(1)
      )
      return(sum(Ws))
    }
    LB <- pmax(min(ests) - .5, -2 + .1^3)
    UB <- max(ests) + .5
    opt <- stats::optim(
    ...
    for (l in levels(fctr)) {
      check_empirical_optimization(calc_null_p_kurtosis(x[fctr == l], opt$par))
    }
  ...
  for (i in seq_along(levels(fctr))) {
    ...
    tempCI <- LRTesteR::empirical_kurtosis_one_sample(tempX, obs_kurtosis, "two.sided", individual.conf.level)
