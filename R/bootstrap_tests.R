...
bootstrap_one_sample <- function(x, param, param.fun, alternative = "two.sided", conf.level = .95) {
  ...
  if (!inherits(param.fun, "function")) {
    stop("Argument param.fun must be a function.")
  }
  args <- names(formals(param.fun))
  if (args[1] != "x") {
    stop("param.fun's first argument is not x.")
  }
  if (length(args) != 1) {
    stop("param.fun has too many arguments.")
  }
  rm(args)
  ...
  generate_param_distribution <- function(B) {
    ...
    for (i in seq(1, B, 1)) {
      ...
      paramStar[i] <- param.fun(xStar)
    }
    return(paramStar)
  }
  paramStar <- generate_param_distribution(B)

  calc_test_stat <- function(x, param, param.fun, alternative) {
  ...
  W <- calc_test_stat(x, param, param.fun, alternative)
